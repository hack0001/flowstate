  ...
  return (
    <main style={{ minHeight: '100vh', maxWidth: '48rem', margin: '0 auto', padding: '2rem 1.5rem', background: C.bg }}>
        ...
        {stages.map((stage, idx) => {
          ...
          return (
            <button key={stage.id} onClick={() => { sounds.playClick(); setActiveStageIdx(idx) }}
              style={{ display: 'inline-flex', alignItems: 'center', gap: '0.375rem', padding: '0.375rem 0.875rem', borderRadius: '9999px', fontSize: '0.875rem', fontWeight: 500, border: '1px solid ' + (done ? 'rgba(0,255,136,0.4)' : active ? C.cyan : C.border), background: done ? 'rgba(0,255,136,0.1)' : active ? 'rgba(0,212,255,0.1)' : C.surface, color: done ? C.green : active ? C.cyan : C.textSec, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.2s' }}>
            ...
            <div key={task.id} style={{ background: done ? 'rgba(0,255,136,0.03)' : C.card, border: '1px solid ' + (done ? 'rgba(0,255,136,0.3)' : expanded ? C.cyan : C.border), borderRadius: '1rem', overflow: 'hidden', transition: 'all 0.2s' }}>
              <div onClick={() => { sounds.playClick(); setExpandedTask(expanded ? null : task.id) }}
                ...
                <div onClick={e => { e.stopPropagation(); toggleTask(task) }}
                  style={{ width: '1.25rem', height: '1.25rem', borderRadius: '0.375rem', border: '2px solid ' + (done ? C.green : C.border), background: done ? C.green : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, cursor: 'pointer', transition: 'all 0.2s', boxShadow: done ? '0 0 10px rgba(0,255,136,0.4)' : 'none' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ fontSize: '0.75rem', color: C.textSec }}>{task.description}{task.estimated_minutes ? ' ~' + task.estimated_minutes + 'm' : ''}</p>
                </div>
                ...
                <div style={{ padding: '0 1rem 1rem', borderTop: '1px solid ' + C.border }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem', flexWrap: 'wrap' }}>
                    ...
                    <button onClick={() => toggleTask(task)}
                      style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '0.375rem', fontSize: '0.75rem', padding: '0.375rem 0.75rem', borderRadius: '0.5rem', background: 'rgba(0,255,136,0.15)', color: C.green, border: '1px solid rgba(0,255,136,0.3)', cursor: 'pointer', fontFamily: 'inherit' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <button onClick={() => { sounds.playClick(); setActiveStageIdx(i => Math.max(0, i - 1)) }} disabled={activeStageIdx === 0}
          style={{ padding: '0.6rem 1.2rem', background: 'transparent', border: '1px solid ' + C.border, borderRadius: '0.75rem', color: C.textSec, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600, opacity: activeStageIdx === 0 ? 0.3 : 1 }}>
          &larr; Previous
        </button>
        <button onClick={() => { sounds.playClick(); setActiveStageIdx(i => Math.min(stages.length - 1, i + 1)) }} disabled={activeStageIdx === stages.length - 1}
          style={{ padding: '0.6rem 1.2rem', background: 'transparent', border: '1px solid ' + C.border, borderRadius: '0.75rem', color: C.textSec, cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600, opacity: activeStageIdx === stages.length - 1 ? 0.3 : 1 }}>
          Next &rarr;
        </button>
      </div>
      <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
