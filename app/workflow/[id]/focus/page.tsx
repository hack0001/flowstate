...
const TIPS = [
  ...
  { tip: 'Your phone can wait. This cannot.', science: 'Average attention recovery after distraction: 25 min.' },
]
    ...
    <main style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: C.bg }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1rem 1.5rem', borderBottom: '1px solid ' + C.border, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          ...
          <span style={{ fontSize: '0.875rem', color: C.textSec }}>{session?.title}</span>
        </div>
      ...
      <div style={{ padding: '0.75rem 1.5rem', borderBottom: '1px solid ' + C.border }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.375rem' }}>
          <span style={{ fontSize: '0.75rem', color: C.textMut }}>{currentStage?.name}</span>
      ...
      <div style={{ flex: 1, display: 'flex', gap: '1.5rem', padding: '1.5rem', minHeight: 0, flexWrap: 'wrap' }}>
          ...
          {task ? (
            <>
              <div style={{ background: isDone ? 'rgba(0,255,136,0.04)' : C.card, border: '1px solid ' + (isDone ? 'rgba(0,255,136,0.4)' : C.border), borderRadius: '1rem', padding: '1.5rem' }}>
                ...
                <p style={{ fontSize: '0.875rem', color: C.textSec }}>{task.description}{task.estimated_minutes ? <span style={{ marginLeft: '0.5rem', padding: '0.1rem 0.4rem', borderRadius: '0.25rem', background: C.surface, color: C.textMut, fontSize: '0.75rem' }}>~{task.estimated_minutes}m</span> : null}</p>
              </div>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1.5rem', paddingTop: '1rem', borderTop: '1px solid ' + C.border, flexWrap: 'wrap' }}>
                    {task.has_prompt && task.prompt_text && (
                      <button onClick={() => { navigator.clipboard.writeText(task.prompt_text!); sounds.playClick(); setCopied(true); setTimeout(() => setCopied(false), 2000) }}
                        style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', fontSize: '0.875rem', padding: '0.5rem 1rem', borderRadius: '0.75rem', background: copied ? 'rgba(0,255,136,0.15)' : 'rgba(139,92,246,0.1)', color: copied ? C.green : C.purple, border: '1px solid ' + (copied ? 'rgba(0,255,136,0.3)' : 'rgba(139,92,246,0.3)'), cursor: 'pointer', fontFamily: 'inherit' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                <button onClick={() => { sounds.playClick(); setTaskIdx(i => Math.max(0, i - 1)) }} disabled={taskIdx === 0}
                  style={{ padding: '0.875rem', background: 'transparent', border: '1px solid ' + C.border, borderRadius: '0.75rem', color: C.textSec, cursor: 'pointer', display: 'flex', alignItems: 'center', opacity: taskIdx === 0 ? 0.3 : 1 }}>
                  <ChevronLeft size={20} />
                </button>
                ...
                <button onClick={() => { sounds.playClick(); setTaskIdx(i => Math.min(allTasks.length - 1, i + 1)) }} disabled={taskIdx === allTasks.length - 1}
                  style={{ padding: '0.875rem', background: 'transparent', border: '1px solid ' + C.border, borderRadius: '0.75rem', color: C.textSec, cursor: 'pointer', display: 'flex', alignItems: 'center', opacity: taskIdx === allTasks.length - 1 ? 0.3 : 1 }}>
        ...
        <div style={{ width: '280px', flexShrink: 0, display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div style={{ background: C.card, border: '1px solid ' + C.border, borderRadius: '1rem', padding: '1.25rem', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: '0.375rem', marginBottom: '1rem' }}>
              {[0,1,2,3].map(i => (
                <div key={i} style={{ width: '0.5rem', height: '0.5rem', borderRadius: '50%', background: i < (pom.pomodorosCompleted % 4) ? C.cyan : C.border }} />
              ))}
            </div>
            <div style={{ display: 'flex', gap: '0.5rem', width: '100%' }}>
              ...
              <button onClick={() => { sounds.playClick(); pom.reset() }} style={{ padding: '0.5rem 0.75rem', background: 'transparent', border: '1px solid ' + C.border, borderRadius: '0.75rem', color: C.textSec, cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
                <RefreshCw size={14} />
              </button>
              <button onClick={() => { sounds.playClick(); pom.skip() }} style={{ padding: '0.5rem 0.75rem', background: 'transparent', border: '1px solid ' + C.border, borderRadius: '0.75rem', color: C.textSec, cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
                <SkipForward size={14} />
              </button>
            </div>
            {pom.isBreak && (
                ...
                return (
                  <div key={stage.id}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.25rem' }}>
                      <span style={{ fontSize: '0.75rem', color: active ? C.cyan : C.textMut, fontWeight: active ? 600 : 400 }}>{stage.name}</span>
