...
function groupColor(g: string): string { return GROUP_COLORS[g] ?? C.purple }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <img src={faviconSrc} alt="" width={20} height={20}
...
function AddModal({ onClose, onSave, existingGroups }:{
  onClose:()=>void
  onSave:(partial:Partial<SavedTab>)=>void
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div>
          ...
          <input ref={urlRef} value={url} onChange={e => setUrl(e.target.value)} placeholder="https://..." style={inpStyle} />
        </div>
        <div>
          ...
          <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Leave blank to auto-generate" style={inpStyle} />
        </div>
        ...
        <div>
          ...
          <input value={group} onChange={e => setGroup(e.target.value)} placeholder="Or type a new group..."
            style={inpStyle} />
        </div>
...
function BookmarkletPanel({ origin, onClose }:{ origin:string; onClose:()=>void }) {
  ...
  // Bookmarklet opens a tiny popup pointing at the app with query params.
  // The popup page auto-saves to localStorage and closes itself.
  ...
  const stepStyle: React.CSSProperties = { display:'flex', gap:'0.75rem', alignItems:'flex-start' }
    ...
    <div style={{ background:C.card, border:'1px solid '+C.cyan+'33', borderRadius:'1rem', padding:'1.25rem', marginBottom:'1.5rem' }}>
      ...
      <div style={{ display:'flex', flexDirection:'column' as const, gap:'0.75rem', marginBottom:'1.1rem' }}>
        {[
          ...
          { n:'4', text:'Click it on any page &mdash; a tiny popup opens, saves the tab, and closes itself automatically' },
        ].map(s => (
      ...
      <div style={{ background:C.surface, border:'1px solid '+C.border, borderRadius:'0.6rem', padding:'0.6rem 0.75rem', marginBottom:'0.75rem' }}>
        <p style={{ fontSize:'0.62rem', color:C.muted, margin:0, fontFamily:'monospace', wordBreak:'break-all' as const, lineHeight:1.5 }}>
          {bm.slice(0, 130)}&hellip;
        </p>
      </div>
...
// ── Popup Save View ─────────────────────────────────────────────────────────
// Shown when the page is opened as a bookmarklet popup (?add=URL)
function PopupSave({ url, title, favicon }: { url:string; title:string; favicon:string }) {
  ...
  useEffect(() => {
    const tabs = readTabs()
    const isDupe = tabs.some(t => t.url === url && t.status === 'active')
    if (!isDupe) {
      let hostname = ''
  ...
  const col = duplicate ? C.amber : C.green
  ...
  return (
    <main style={{
      ...
      <div style={{ textAlign:'center' as const }}>
        ...
        <p style={{ fontSize:'0.9rem', fontWeight:800, color:col, margin:'0 0 0.25rem' }}>
          {duplicate ? 'Already saved' : 'Saved to Tab Sheet'}
        </p>
        <p style={{ fontSize:'0.72rem', color:C.muted, margin:0 }}>{domain}</p>
        <p style={{ fontSize:'0.62rem', color:C.muted, marginTop:'0.5rem' }}>Closing&hellip;</p>
      </div>
    </main>
  )
}
...
function TabsInner() {
  ...
  // Bookmarklet popup detection — ?add= query param
  ...
  // If this is a popup save request, render the mini save view
  if (addUrl) {
  ...
  function saveTab(partial: Partial<SavedTab>) {
    ...
    if (existing.some(t => t.url === partial.url && t.status === 'active')) return
    let hostname = ''
    try { hostname = new URL(partial.url || '').hostname } catch {}
    const newTab: SavedTab = {
      id: `tab_${Date.now()}_${Math.random().toString(36).slice(2,6)}`,
      url: partial.url || '',
      title: partial.title || hostname || 'Untitled',
      favicon: partial.favicon || `https://www.google.com/s2/favicons?domain=${hostname}&sz=32`,
      group: partial.group || '',
      notes: partial.notes || '',
      addedAt: new Date().toISOString(),
      status: 'active',
    ...
    const next = [newTab, ...existing]
    writeTabs(next)
    setTabs(next)
  }

  function archiveTab(id: string) {
    const next = readTabs().map(t => t.id === id ? { ...t, status:'archived' as const } : t)
    writeTabs(next); setTabs(next)
  }

  function restoreTab(id: string) {
    const next = readTabs().map(t => t.id === id ? { ...t, status:'active' as const } : t)
    writeTabs(next); setTabs(next)
  }

  function deleteTab(id: string) {
    const next = readTabs().filter(t => t.id !== id)
    writeTabs(next); setTabs(next)
  }
    ...
    <main style={{ minHeight:'100vh', background:C.bg, color:C.text }}>
      ...
      <div style={{ maxWidth:'800px', margin:'0 auto', padding:'1.5rem 2rem', opacity:mounted?1:0, transition:'opacity 0.3s', animation:'fadeInUp 0.3s ease both' }}>
        ...
        <div style={{ display:'flex', gap:'0.5rem', marginBottom:'0.875rem', flexWrap:'wrap' as const }}>
          ...
          <button onClick={() => setShowBM(v => !v)} style={{
            ...
            padding:'0.45rem 0.875rem', background:C.card,
            border:'1px solid '+(showBM ? C.cyan+'44' : C.border),
            ...
            {view==='active' && activeTabs.length===0 ? (
              <>
                ...
                <p style={{ fontSize:'0.78rem', color:C.muted, margin:'0 0 1.25rem', lineHeight:1.6 }}>
                  Add tabs manually with the + button, or set up the<br />bookmarklet to save any Chrome tab in one click.
                </p>
      ...
      {showAdd && (
        <AddModal
          ...
          onSave={p => { saveTab(p); setShowAdd(false) }}
