...
function fromRow(r: any): SavedTab {
  return { id:r.id, url:r.url, title:r.title, favicon:r.favicon, group:r.tab_group??'', notes:r.notes, addedAt:r.added_at, status:r.status, source:r.source }
}
function toRow(t: SavedTab) {
  return { id:t.id, url:t.url, title:t.title, favicon:t.favicon, tab_group:t.group, notes:t.notes, added_at:t.addedAt, status:t.status, source:t.source }
}

// ── Local storage helpers ────────────────────────────────────────────────────
function lsRead(): SavedTab[] {
  try { return JSON.parse(localStorage.getItem(LS_KEY) || '[]') } catch { return [] }
}
function lsWrite(tabs: SavedTab[]) {
  try { localStorage.setItem(LS_KEY, JSON.stringify(tabs)) } catch {}
}

// ── Supabase background sync ─────────────────────────────────────────────────
// Pulls from Supabase, merges with local, pushes any local-only rows back.
// Never blocks the UI — called fire-and-forget.
async function dbSync(localTabs: SavedTab[], onMerge: (merged: SavedTab[]) => void) {
  try {
    ...
    const localOnly  = localTabs.filter(t => !remoteIds.has(t.id))

    // Push any tabs that exist locally but not yet in the DB
    if (localOnly.length > 0) {
      supabase.from('tabs').upsert(localOnly.map(toRow), { onConflict: 'id' }).then()
    }

    // Merged list: remote is authoritative for shared rows; local-only rows appended
    ...
    // Supabase unavailable — stay on localStorage, silently ignore
  }
}
...
function TabCard({ tab, archived, onOpen, onArchive, onRestore, onDelete }:{
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:'0.75rem', padding:'0.75rem 1rem', background:archived?'rgba(255,255,255,0.01)':C.card, border:'1px solid '+(archived?C.muted+'44':C.border), borderRadius:'0.75rem', opacity:archived?0.7:1 }}>
      ...
      <div style={{ flex:1, minWidth:0 }}>
        <p style={{ fontSize:'0.82rem', fontWeight:600, color:archived?C.sec:C.text, margin:0, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', textDecoration:archived?'line-through':'none' }}>{tab.title||domain}</p>
...
function AddModal({ onClose, onSave, existingGroups }:{ onClose:()=>void; onSave:(p:Partial<SavedTab>)=>void; existingGroups:string[] }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.7)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000, padding:'1rem' }} onClick={e=>{ if(e.target===e.currentTarget) onClose() }}>
    ...
    <div style={{ background:C.card, border:'1px solid '+C.cyan+'33', borderRadius:'1rem', padding:'1.25rem', marginBottom:'1.5rem' }}>
      ...
      <div style={{ display:'flex', flexDirection:'column' as const, gap:'0.75rem', marginBottom:'1.1rem' }}>
        {[
          ...
          { n:'4', text:'Click it on any page to save that tab &mdash; synced across all your devices' },
        ].map(s=>(
  ...
  useEffect(()=>{
    async function save() {
      try {
        ...
        const local = lsRead()
        const isDupe = local.some(t => t.url === url && t.status === 'active')
        if (isDupe) { setState('duplicate'); setTimeout(()=>{ try{ window.close() }catch{} },1800); return }

        const next = [newTab, ...local]
        lsWrite(next)
        setState('saved')
...
function TabsInner() {
  ...
  useEffect(()=>{
    ...
    // 1. Load from localStorage immediately — no loading state needed
    const local = lsRead()
    setTabs(local)
    setMounted(true)
  ...
  // Bookmarklet popup — show mini save view instead of full UI
  if (mounted && addUrl) {
  ...
  function mutate(id: string, updates: Partial<SavedTab>) {
    ...
    supabase.from('tabs').update(
      Object.fromEntries(Object.entries(updates).map(([k,v]) => [k==='group'?'tab_group':k==='addedAt'?'added_at':k, v]))
    ).eq('id', id).then()
  }
    ...
    <main style={{ minHeight:'100vh', background:C.bg, color:C.text }}>
      ...
      <div style={{ background:C.surface, borderBottom:'1px solid '+C.border, padding:'1.25rem 2rem' }}>
        <div style={{ maxWidth:'800px', margin:'0 auto' }}>
          ...
          <p style={{ fontSize:'0.78rem', color:C.sec, margin:'0.2rem 0 0' }}>Saves locally first, syncs across devices in the background</p>
        </div>
      </div>
      ...
      {showAdd && <AddModal onClose={()=>setShowAdd(false)} onSave={p=>{ saveTab(p); setShowAdd(false) }} existingGroups={allGroups}/>}
    </main>
  )
}
